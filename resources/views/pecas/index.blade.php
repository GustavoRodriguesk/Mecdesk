                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="number" name="estoque_min" value="{{ request('estoque_min') }}"
                                    placeholder="Mínimo"
                                    class="search-input w-full px-3 py-2 text-sm border border-gray-300 rounded-md bg-white text-gray-900 placeholder-gray-400 focus:outline-none focus:border-blue-500 transition-colors duration-150">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" name="estoque_max" value="{{ request('estoque_max') }}"
                                    placeholder="Máximo"
                                    class="search-input w-full px-3 py-2 text-sm border border-gray-300 rounded-md bg-white text-gray-900 placeholder-gray-400 focus:outline-none focus:border-blue-500 transition-colors duration-150">
                            </div>
                    ...
                    <div class="flex items-center justify-between mt-2">
                        <div class="flex items-center gap-2 text-sm text-gray-400">
                            ...
                            @if (isset($pecas) && method_exists($pecas, 'total'))
                                <span class="ml-2 text-gray-500">{{ $pecas->total() }}
                                    {{ $pecas->total() === 1 ? 'peça' : 'peças' }}</span>
                            @endif
                        </div>
                                ...
                                <td colspan="5" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center gap-3 text-gray-400">
                                        ...
                                        @if (request('search') || request('nome') || request('codigo') || request('estoque_min') || request('estoque_max'))
                                            <p class="text-sm font-medium text-gray-500">Nenhuma peça encontrada com os
                                                filtros aplicados</p>
                                            <a href="{{ route('pecas.index') }}"
                                                class="text-xs text-blue-600 hover:underline">Limpar filtros e ver
                                                todas</a>
                                        @else
                                            <p class="text-sm font-medium text-gray-500">Nenhuma peça cadastrada ainda
                                            </p>
                                            <a href="{{ route('pecas.create') }}"
                                                class="text-xs text-blue-600 hover:underline">Cadastrar primeira
                                                peça</a>
                                        @endif
                                    </div>
                                </td>
